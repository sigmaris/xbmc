...
{
  std::list<struct crtc_option> crtcs = FindCrtcOptions();
  if (crtcs.empty())
  {
  ...
  // sorts crtcs from least to most suitable
  crtcs.sort(CDRMUtils::CompareCrtcOptions);

  struct crtc_option &best_crtc = crtcs.back();

  delete m_crtc;
  m_crtc = best_crtc.crtc_obj;
  m_video_plane->plane = best_crtc.video_plane;
  m_gui_plane->plane = best_crtc.gui_plane;

  if (!best_crtc.gui_10bit)
...
bool CDRMUtils::CompareCrtcOptions(const struct crtc_option &l, const struct crtc_option &r)
{
  /* Prefer CRTC options in this order: has video plane, has 10bit GUI */
  return std::make_tuple(l.video_plane != nullptr, l.gui_10bit)
       < std::make_tuple(r.video_plane != nullptr, r.gui_10bit);
}

std::list<struct crtc_option> CDRMUtils::FindCrtcOptions()
{
  std::list<struct crtc_option> crtcs;
